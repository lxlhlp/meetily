 ...
 */
export async function showRecordingNotification(t: TranslateFn): Promise<void> {
  try {
    ...
    if (showNotification) {
      ...
      const toastId = toast.info(t('home.recordingNotificationTitle'), {
        description: (
          <div className="space-y-3 min-w-[280px]">
            <p className="text-sm font-medium text-gray-900">
              {t('home.recordingNotificationDesc')}
            </p>
            <label className="flex items-center gap-2 text-xs cursor-pointer hover:bg-blue-100 p-2 rounded transition-colors">
              ...
              />
              <span className="select-none text-gray-700">{t('home.recordingNotificationDontShow')}</span>
            </label>
            <button
              onClick={async () => {
                if (dontShowAgain) {
            ...
            >
              {t('home.recordingNotificationAck')}
            </button>
          </div>
        ),
        duration: 10000,
