...
export function DeviceSelection({ selectedDevices, onDeviceChange, disabled = false }: DeviceSelectionProps) {
  ...
  const fetchDevices = async () => {
    try {
      ...
      setError(t('device.loadDevicesFailed'));
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const startAudioLevelMonitoring = async () => {
    try {
      ...
      if (deviceNames.length === 0) {
        setError(t('device.noMicToMonitor'));
        return;
      }
      ...
      setError(t('device.monitorStartFailed'));
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-gray-900">{t('device.audioDevices')}</h4>
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger id="mic-selection" className="w-full">
              <SelectValue placeholder={t('device.selectMicrophone')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="default">{t('device.defaultMicrophone')}</SelectItem>
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-gray-500">{t('device.noMicFound')}</p>
          )}

          {/* Audio Level Meters for Input Devices */}
          {showLevels && inputDevices.length > 0 && (
            <div className="space-y-2 pt-2 border-t border-gray-100">
              <p className="text-xs text-gray-600 font-medium">{t('device.micLevels')}</p>
              {inputDevices.map((device) => {
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger id="system-selection" className="w-full">
              <SelectValue placeholder={t('device.selectSystemAudio')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="default">{t('device.defaultSystemAudio')}</SelectItem>
          ...
          {outputDevices.length === 0 && (
            <p className="text-xs text-gray-500">{t('device.noSystemAudioFound')}</p>
          )}
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>{t('device.helpMic')}</p>
        <p>{t('device.helpSystem')}</p>
        {isMonitoring && (
          <p>{t('device.helpLevels')}</p>
        )}
        {!isMonitoring && inputDevices.length > 0 && (
          <p>{t('device.helpTip')}</p>
        )}
      </div>
    </div>
  );
