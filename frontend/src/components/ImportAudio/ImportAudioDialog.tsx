...
export function ImportAudioDialog({
  ...
  const handleImportError = useCallback((error: string) => {
    toast.error(t('importAudio.importFailed'), { description: error });
  }, [t]);
  ...
  const handleCancel = async () => {
    if (isProcessing) {
      await cancelImport();
      toast.info(t('importAudio.importCancelled'));
    }
    onOpenChange(false);
  };
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      >
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            {isProcessing ? (
              <>
                ...
                {t('importAudio.importing')}
              </>
            ) : error ? (
              <>
                <AlertCircle className="h-5 w-5 text-red-600" />
                {t('importAudio.importFailed')}
              </>
            ) : status === 'complete' ? (
              <>
                <CheckCircle2 className="h-5 w-5 text-green-600" />
                {t('importAudio.importCompleteTitle')}
              </>
            ) : (
              <>
          ...
          <DialogDescription>
            {isProcessing
              ? progress?.message || t('importAudio.processingAudio')
              : error
              ? t('importAudio.importErrorOccurred')
              : t('importAudio.importHint')}
          </DialogDescription>
        </DialogHeader>
                ...
                <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      placeholder={t('importAudio.enterMeetingTitle')}
                    />
                  </div>
                    ...
                    <div className="p-3 pt-0 space-y-4 border-t">
                      ...
                      {availableModels.length > 0 && (
                        <div className="space-y-2">
                          ...
                          >
                            <SelectTrigger className="w-full">
                              <SelectValue placeholder={loadingModels ? t('importAudio.loadingModels') : t('importAudio.selectModel')} />
                            </SelectTrigger>
                            <SelectContent>
