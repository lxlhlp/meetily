...
export const AISummary = ({ summary, status, error, onSummaryChange, onRegenerateSummary, meeting }: Props) => {
  ...
  const currentSummary = useMemo(() => {
    if (!summary) {
      return {
        Agenda: { title: t('aiSummary.agenda'), blocks: [] },
        Decisions: { title: t('meeting.decisions'), blocks: [] },
        ActionItems: { title: t('meeting.actionItems'), blocks: [] },
        ClosingRemarks: { title: t('aiSummary.closingRemarks'), blocks: [] }
      };
    }
  ...
  const renderErrorState = () => (
    <div className="w-full p-4 bg-red-50 border border-red-200 rounded-lg">
      <div className="flex items-center mb-2">
        ...
        <h3 className="text-red-700 font-medium">{t('summary.errorGenerating')}</h3>
      </div>
      <p className="text-red-600 text-sm">{error}</p>
      <p className="text-red-500 text-xs mt-2">{t('aiSummary.checkModelConfig')}</p>
    </div>
  );
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h3 className="text-blue-700 font-medium">
            {t(status === 'processing' ? 'summary.processingTranscript' : 'summary.generatingSummary')}
          </h3>
          <p className="text-blue-600 text-sm">
            {status === 'processing' 
              ? t('aiSummary.analyzingTranscript') 
              : t('aiSummary.creatingDetailedSummary')}
          </p>
        </div>
      </div>
    </div>
  ...
  if (!hasContent && status === 'completed') {
    return (
      <div className="w-full p-4 bg-gray-50 border border-gray-200 rounded-lg text-center">
        <p className="text-gray-600">{t('aiSummary.noContentAvailable')}</p>
        <p className="text-gray-500 text-sm mt-1">{t('aiSummary.tryGenerateNew')}</p>
      </div>
    );
  }
