  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm relative">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="text-lg font-semibold text-gray-900">{t('settings.summaryLanguageSettings')}</h3>
      </div>
      <p className="text-sm text-gray-600 mb-4">
        {t('settings.summaryLanguageSettingsDesc')}
      </p>
        ...
        {recents.map((code) => {
          ...
          return (
            ...
            >
              <button
                type="button"
                aria-label={t('settings.pinLanguage', { label: labelForCode(code) })}
                aria-pressed={isPinned}
                title={t('settings.unsetDefault')}
              ...
              <button
                type="button"
                aria-label={t('settings.removeLanguage', { label: labelForCode(code) })}
      ...
      <p className="text-xs text-gray-400 mt-3">
        {pinned
          ? t('settings.defaultLanguageHint', { label: labelForCode(pinned) })
          : t('settings.pickLanguageHint')}
      </p>
    </div>
  );
}
