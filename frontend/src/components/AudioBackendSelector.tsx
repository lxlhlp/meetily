...
export function AudioBackendSelector({
  ...
  useEffect(() => {
    const loadBackends = async () => {
      try {
        ...
        setError(t('settings.backendLoadFailed'));
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleBackendChange = async (backendId: string) => {
    try {
      ...
      setError(t('settings.backendChangeFailed'));
    }
  };
  ...
  return (
    <div className="space-y-2">
        ...
        <div className="relative">
          ...
          {showTooltip && (
            <div className="absolute z-10 left-6 top-0 w-64 p-3 text-xs bg-gray-900 text-white rounded-lg shadow-lg">
              <p className="font-semibold mb-1">{t('settings.audioCaptureMethods')}</p>
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>{t('settings.backendBulletSystemOnly')}</p>
        <p>{t('settings.backendBulletMicDefault')}</p>
        <p>{t('settings.backendBulletNewSessions')}</p>
      </div>
    </div>
  );
}