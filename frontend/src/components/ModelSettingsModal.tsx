...
export function ModelSettingsModal({
  ...
  // Reasoning/thinking mode for reasoning models (e.g. Qwen) - default on,
  // matching both the Qwen server default and historical behavior
  const [customEnableThinking, setCustomEnableThinking] = useState<boolean>(modelConfig.enableThinking ?? true);
          ...
          if (data.provider === 'custom-openai') {
            try {
              ...
              if (customConfig) {
                ...
                setCustomEnableThinking(customConfig.enableThinking ?? true);
              }
            } catch (err) {
  ...
  useEffect(() => {
    if (modelConfig.provider === 'custom-openai') {
      ...
      setCustomEnableThinking(modelConfig.enableThinking ?? true);
    }
  }, [
  ...
  const fetchOllamaModels = async (silent = false) => {
    ...
    if (trimmedEndpoint && !validateOllamaEndpoint(trimmedEndpoint)) {
      const errorMsg = t('modelSettings.invalidEndpointUrl');
      setError(errorMsg);
      if (!silent) {
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : t('modelSettings.loadOllamaModelsFailed');
      setError(errorMsg);
  ...
  const loadOpenRouterModels = async () => {
    ...
    try {
      ...
      setOpenRouterError(
        err instanceof Error ? err.message : t('modelSettings.loadOpenRouterModelsFailed')
      );
    } finally {
  ...
  const loadBuiltinAiModels = async () => {
    ...
    try {
      ...
      toast.error(t('modelSettings.loadBuiltinModelsFailed'));
    }
  };
  ...
  const handleSave = async () => {
    ...
    const updatedConfig = {
      ...
      enableThinking: modelConfig.provider === 'custom-openai' ? customEnableThinking : null,
  ...
  const testCustomOpenAIConnection = async () => {
    if (!customOpenAIEndpoint.trim() || !customOpenAIModel.trim()) {
      toast.error(t('modelSettings.enterEndpointAndModel'));
      return;
    }
    ...
    try {
      ...
      toast.success(result.message || t('modelSettings.connectionSuccessful'));
    } catch (err) {
  ...
  const downloadRecommendedModel = async () => {
    ...
    if (isDownloading(recommendedModel)) {
      toast.info(t('modelSettings.alreadyDownloading', { model: recommendedModel }), {
        description: t('modelSettings.downloadProgress', { progress: Math.round(getProgress(recommendedModel) || 0) })
      });
      return;
    }
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : t('modelSettings.downloadModelFailed');
      console.error('Error downloading model:', err);

      // Check if Ollama is not installed and show appropriate error
      if (isOllamaNotInstalledError(errorMsg)) {
        toast.error(t('modelSettings.ollamaNotInstalled'), {
  ...
  const deleteOllamaModel = async (modelName: string) => {
    try {
      ...
      toast.success(t('modelSettings.modelDeleted', { model: modelName }));
      await fetchOllamaModels(true); // Refresh list
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : t('modelSettings.deleteModelFailed');
      toast.error(errorMsg);
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        <div>
          <Label>{t('modelSettings.summarizationModel')}</Label>
          <div className="flex space-x-2 mt-1">
            ...
            >
              ...
              <SelectContent className="max-h-64 overflow-y-auto">
                <SelectItem value="builtin-ai">{t('modelSettings.providerBuiltinAi')}</SelectItem>
                <SelectItem value="claude">Claude</SelectItem>
                <SelectItem value="custom-openai">{t('modelSettings.providerCustomOpenAi')}</SelectItem>
            ...
            {modelConfig.provider !== 'builtin-ai' && modelConfig.provider !== 'custom-openai' && (
              <Popover open={modelComboboxOpen} onOpenChange={setModelComboboxOpen} modal={true}>
                <PopoverTrigger asChild>
                  ...
                  >
                    <span className="truncate">
                      {modelConfig.model || t('modelSettings.selectModel')}
                    </span>
                ...
                <PopoverContent className="w-[250px] p-0" align="start">
                  <Command>
                    <CommandInput placeholder={t('modelSettings.searchModels')} />
          ...
          <div className="space-y-4 border-t pt-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {t('modelSettings.endpointUrlDesc')}
              </p>
            </div>
                ...
                <div className="space-y-3 pl-2 border-l-2 border-muted mt-2">
                  <div>
                    ...
                    <Input
                      ...
                      placeholder={t('modelSettings.maxTokensPlaceholder')}
                      className="mt-1"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      placeholder={t('modelSettings.temperaturePlaceholder')}
                      className="mt-1"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      placeholder={t('modelSettings.topPPlaceholder')}
                      className="mt-1"
                    />
                  </div>
                  <div className="flex items-center justify-between pt-1">
                    <div className="space-y-0.5">
                      <Label htmlFor="custom-enable-thinking">{t('modelSettings.reasoningMode')}</Label>
                      <p className="text-xs text-muted-foreground">{t('modelSettings.reasoningModeDesc')}</p>
                    </div>
                    <Switch
                      id="custom-enable-thinking"
                      checked={customEnableThinking}
                      onCheckedChange={setCustomEnableThinking}
                    />
                  </div>
                </div>
              )}
        ...
        {modelConfig.provider === 'ollama' && (
          <div>
            ...
            >
              <Label className="cursor-pointer">{t('modelSettings.customEndpointOptional')}</Label>
            ...
            {!isEndpointSectionCollapsed && (
              <>
                <p className="text-sm text-muted-foreground mt-1 mb-2">
                  {t('modelSettings.customEndpointDesc')}
                </p>
                ...
                {ollamaEndpointChanged && !error && (
                  <Alert className="mt-3 border-yellow-500 bg-yellow-50">
                    <AlertDescription className="text-yellow-800">
                      {t('modelSettings.endpointChangedAlert')}
                    </AlertDescription>
                  </Alert>
                )}
              </>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              {lastFetchedEndpoint && models.length > 0 && (
                <div className="flex items-center gap-2 text-sm">
                  <span className="text-muted-foreground">{t('modelSettings.using')}</span>
            ...
            {isLoadingOllama ? (
                  ...
                  <div className="space-y-4">
                    <Alert className="border-orange-500 bg-orange-50">
                      <AlertDescription className="text-orange-800">
                        {t('modelSettings.ollamaNotRunningAlert')}
                      </AlertDescription>
                    </Alert>
                    <Button
                      variant="default"
                    ...
                    <div className="text-sm text-muted-foreground text-center">
                      {t('modelSettings.afterInstallHint')}
                    </div>
                  </div>
                ) : (
                    ...
                    <Alert className="mb-4">
                      <AlertDescription>
                        {ollamaEndpointChanged
                          ? t('modelSettings.endpointChangedShort')
                          : t('modelSettings.noModelsDownloadHint')}
                      </AlertDescription>
                    </Alert>
                    {!ollamaEndpointChanged && (
                      <div className="space-y-3">
                        ...
                        >
                          {isDownloading('gemma3:1b') ? (
                            <>
                              <RefreshCw className="mr-2 h-4 w-4 animate-spin" />
                              {t('modelSettings.downloadingRecommended')}
                            </>
                          ) : (
                            <>
                              <Download className="mr-2 h-4 w-4" />
                              {t('modelSettings.downloadRecommended')}
                            </>
                          )}
                        </Button>
                        ...
                        {isDownloading('gemma3:1b') && getProgress('gemma3:1b') !== undefined && (
                          <div className="bg-white rounded-md border p-3">
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-sm font-medium text-blue-600">{t('modelSettings.downloadingRecommendedTitle')}</span>
              ...
              <ScrollArea className="max-h-[calc(100vh-450px)] overflow-y-auto pr-4">
                {filteredModels.length === 0 ? (
                  <Alert>
                    <AlertDescription>
                      {t('modelSettings.noSearchMatch', { query: searchQuery })}
                    </AlertDescription>
                  </Alert>
                ) : (
                        ...
                        >
                          <div>
                            <span className="font-bold">
                              {t('modelSettings.modelWithSize', { name: model.name, size: model.size })}
                            </span>
                          </div>
                          ...
                          {modelIsDownloading && progress !== undefined && (
                            <div className="mt-3 pt-3 border-t border-gray-200">
                              <div className="flex items-center justify-between mb-2">
                                <span className="text-sm font-medium text-blue-600">{t('modelSettings.downloading')}</span>
