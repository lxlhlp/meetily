...
export function LanguageSelection({
  ...
  const handleLanguageChange = async (languageCode: string) => {
    ...
    try {
      ...
      toast.success(t('meeting.languagePrefSaved'), {
        description: t('meeting.languageSetTo', { language: languageName })
      });
    } catch (error) {
      console.error('Failed to save language preference:', error);
      toast.error(t('meeting.languagePrefSaveFailed'), {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-sm font-medium text-gray-900">{t('meeting.transcriptionLanguage')}</h4>
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        {isParakeet && (
          <div className="p-2 bg-amber-50 border border-amber-200 rounded text-amber-800">
            <p className="font-medium">{t('meeting.parakeetLanguageHint')}</p>
            <p className="mt-1 text-xs">{t('meeting.languageNotSupportedForParakeet')}</p>
          </div>
        )}
        ...
        <div className="text-xs space-y-2 pt-2">
          <p className="text-gray-600">
            <strong>{t('meeting.current')}</strong> {selectedLanguageName}
          </p>
          {selectedLanguage === 'auto' && (
            <div className="p-2 bg-yellow-50 border border-yellow-200 rounded text-yellow-800">
              <p className="font-medium">{t('meeting.autoDetectWarning')}</p>
              <p className="mt-1">{t('meeting.selectLanguageForAccuracy')}</p>
            </div>
          )}
          {selectedLanguage === 'auto-translate' && (
            <div className="p-2 bg-blue-50 border border-blue-200 rounded text-blue-800">
              <p className="font-medium">{t('meeting.translationModeActive')}</p>
              <p className="mt-1">{t('meeting.translationModeDesc')}</p>
            </div>
          )}
          {selectedLanguage !== 'auto' && selectedLanguage !== 'auto-translate' && (
            <p className="text-gray-600">
              {t('meeting.transcriptionOptimizedForPrefix')} <strong>{selectedLanguageName}</strong>{t('meeting.transcriptionOptimizedForSuffix')}
            </p>
          )}
        </div>
      </div>
