...
impl VadSession {
    ...
    pub fn trim_start_silence(&mut self) {
        let last_index = if let Some(start_ms) = self.speech_start_ms {
            ...
            // FIX(vendored): use processed_duration() instead of session_time().
            // session_time() includes frames of the current input window that
            // have not been processed yet - a window may already contain the
            // next utterance, so trimming to `session_time - pad` could advance
            // deleted_samples PAST that utterance's start_ms, making the
            // subsequent SpeechEnd commit panic with "Duration Xms is outside
            // of session audio range" (observed in production at 8.8s and
            // 230s into recordings, freezing live captions).
            let remove_to = self.processed_duration() - self.config.pre_speech_pad;
